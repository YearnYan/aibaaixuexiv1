...
router.post('/figure', async (req, res) => {
  try {
    ...
    if (svgCache.has(payload.cacheKey)) {
      return res.json({ svg: svgCache.get(payload.cacheKey), cached: true });
    }

    const svg = await generateSVG(payload);
    if (!svg) {
      return res.json({ svg: null, cached: false });
    }
  ...
  try {
    ...
    const uncachedResults = await Promise.all(
      uncached.map(async ({ id, payload }) => {
        const svg = await generateSVG(payload).catch(() => null);
        if (svg) {
          svgCache.set(payload.cacheKey, svg);
        }
    ...
    return res.json({ results: [...cached, ...uncachedResults] });
  } catch (error) {
    console.error('批量渲染失败:', error.message);
    return res.status(500).json({ error: '批量渲染失败' });
  }
});
...
async function generateSVG(payload, retryCount = 0) {
  ...
  try {
    ...
    if (retryCount < MAX_RETRIES) {
      console.warn(`[SVG] AI返回无效图形，准备重试 ${retryCount + 1}/${MAX_RETRIES + 1}`);
      await wait(350 * (retryCount + 1));
      return generateSVG(payload, retryCount + 1);
    }
  } catch (error) {
    const maybeRetriable = /timeout|network|429|500|502|503|504/i.test(error.message || '');
    if (retryCount < MAX_RETRIES && maybeRetriable) {
      console.warn(`[SVG] 调用失败，准备重试 ${retryCount + 1}/${MAX_RETRIES + 1}: ${error.message}`);
...
function isRenderableSvg(svg) {
  ...
  const svgOnlyText = visibleGeometryCount === 0 && textCount > 0;
  if (svgOnlyText) return false;
