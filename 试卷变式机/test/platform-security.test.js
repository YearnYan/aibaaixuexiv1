...
test('平台代理请求必须同时满足回环地址、内部令牌和用户身份', () => {
  ...
  assert.equal(valid.request.platformUserId, 'user-1');
...
test('独立运行模式仍要求 Cookie 与请求令牌并扣减本地体验次数', () => {
  ...
  assert.deepEqual(consumeTrialCredit(authenticated.request.securitySession), { allowed: true, remaining: 2 });
});
