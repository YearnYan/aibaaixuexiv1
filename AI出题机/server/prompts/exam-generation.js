const { getDifficultyPrompt } = require('./difficulty-levels');
...
function assembleExamPrompt(params) {
  const {
    version = '全版本融合课程体系', grade, subject, topic, examPoint,
  ...
  return `你是一位资深的${grade}${subject}教师，精通新课标与主流教材共性体系（当前采用：${version}），拥有20年教学经验。
...
【课程体系约束】
- 严格遵循新课标与主流教材共性知识体系
- 题目内容必须与${grade}教学进度和难度匹配
- 不得出现超纲或年级错配的知识点与表述方式
...
module.exports = { assembleExamPrompt, EXAM_TYPE_NAMES };
