...
async function runStrictFailureSuite() {
  ...
  try {
    ...
    assert.ok(response.status >= 500, `无效 SVG 必须返回 5xx 并阻断，实际状态码为 ${response.status}`);
    const data = await response.json();
    assert.ok(data.error, '严格失败响应必须包含明确错误');
    assert.equal(data.svg, undefined, '严格失败响应不得包含 SVG');
  } finally {
    server.close();
...
async function main() {
  ...
  console.log('开始验证图形渲染：严格失败阻断');
  await runStrictFailureSuite();
  console.log('严格失败阻断验证通过');
