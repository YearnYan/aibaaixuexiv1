...
const EXAM_TYPE_NAMES = {
  quiz: '随堂小测', unit: '单元测试', midterm: '期中考试',
  final: '期末考试', mock: '模拟考试'
};
...
const PRACTICE_DEFAULT_TYPE_COUNTS = { similar: 1, variant: 1, application: 1 };
const PRACTICE_COUNT_LIMITS = { min: 1, max: 3 };
const UPLOAD_ALLOWED_MIME_TYPES = ['image/png', 'image/jpeg', 'application/pdf'];
const UPLOAD_MAX_BYTES = 8 * 1024 * 1024;
const TYPE_ORDER = {
  similar: 1, variant: 2, application: 3,
  choice: 10, fill: 11, blank: 11, calculation: 12, qa: 12, other: 99
};

function buildRuntimeConfig() {
  return {
    upload: { maxUploadSizeBytes: UPLOAD_MAX_BYTES, allowedMimeTypes: UPLOAD_ALLOWED_MIME_TYPES },
    practice: {
...
export function examRoutes(app) {
  ...
  app.post('/suggest-topics', async (c) => {
    try {
      ...
      const topics = await generateTopicSuggestions(client, c.env, { version, grade, subject, keyword, imageUrls });
  ...
  // POST /analyze-wrong-question
  app.post('/analyze-wrong-question', async (c) => {
    try {
      const body = await c.req.json();
      const { fileName = '', mimeType = '', previewImageUrl = '', extractedText = '' } = body;
      ...
      const client = createAIClient(c.env);
      const result = await analyzeWrongQuestion(client, c.env, { fileName, mimeType, previewImageUrl, extractedText });
    ...
    try {
      ...
      if (hasPracticePayload) {
        const {
          ...
          answerAnalysis = '', sourceQuestionText = '', sourceQuestionImage = '',
          hasFigure = false, questionTypes = [], questionTypeCounts = {}, questionCount = 3
        } = body;
        ...
        const resolvedQuestionCount = Math.max(1,
          parseInt(questionCount, 10) || totalFromTypeCounts || normalizedTypes.length * 2
        );
      ...
      const {
        version = '人教版', grade, subject, topic, examPoint,
        difficulty = 5, examType = 'final', questionTypes = [],
        questionCount = 15, imageUrls = []
      } = body;
      ...
      const difficultyPrompt = getDifficultyPrompt(parseInt(difficulty, 10), grade, subject);
      ...
      const exam = await generateExam(client, c.env, {
        version, grade, subject, topics: topic,
