...
const FigureSchema = z.object({
  ...
  kind: z.enum(["diagram", "table", "option-group", "option"]).optional(),
  optionLabel: z.string().max(8).optional(),
...
const ExportSchema = z.object({
  title: z.string().default("资料内容重排"),
  questions: z.array(z.object({
    id: z.string(),
    kind: z.enum(["question", "content"]).default("question"),
    number: z.string(),
...
type ExportFigure = z.infer<typeof FigureSchema>;
type VisualOptionFigure = ExportFigure & { optionLabel: string };
...
export async function exportQuestionsToDocx(input: unknown) {
  ...
  for (const question of payload.questions) {
    ...
    for (const [figureIndex, figure] of question.figures.entries()) {
      ...
      const image = dataUrlToBuffer(figure.dataUrl);
  ...
  return injectFormulaOmml(buffer, formulas);
}
...
function isVisualOptionFigure(figure: ExportFigure): figure is VisualOptionFigure {
...
function visualOptionCell(
  ...
  if (entry.figure) {
    const image = dataUrlToBuffer(entry.figure.dataUrl);
...
async function injectFormulaOmml(buffer: Buffer, formulas: FormulaSlot[]) {
  if (!formulas.length) {
    return buffer;
  }
