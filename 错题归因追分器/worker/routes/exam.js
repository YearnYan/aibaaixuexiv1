...
const PRACTICE_DEFAULT_TYPE_COUNTS = { similar: 3, variant: 3, application: 3 };
  ...
  app.post('/analyze-wrong-question', async (c) => {
    try {
      ...
      if (
        ...
        return c.json({ error: '缺少可识别的题目图片或文本数据' }, 400);
      }
      if (!UPLOAD_ALLOWED_MIME_TYPES.includes(mimeType)) {
        return c.json({ error: '仅支持 PNG/JPG/PDF/Word' }, 400);
      }
    ...
    try {
      ...
      if (hasPracticePayload) {
        ...
        const resolvedQuestionCount = Math.max(1,
          parseInt(questionCount, 10) || totalFromTypeCounts || normalizedTypes.length * 3
        );
